                      ...
                      <div class="card-body">
                        <h4 class="card-title pb-4">
                            <div class="row justify-content-between">
                              <div class="col-lg-1">
                                <a href="{{route('admin.manage.index')}}" class="btn btn-primary">Back</a>
                              </div>
                            </div>
                        </h4>
                        ...
                        <form action="" id='FormData' method="POST" enctype="multipart/form-data" >
                          <div class="form-group col-lg-7">
                              ...
                              <input type="text" name="first_name" value="{{ $row->first_name }}"  class="form-control" placeholder="{{ ln('first_name','ucwords') }}">
                          </div>

                          <div class="form-group col-lg-7">
                            <label for="last_name">{{ ln('last_name','ucwords') }}</label>
                            <input type="text" name="last_name"  value="{{ $row->last_name }}" class="form-control" placeholder="{{ ln('last_name','ucwords') }}">
                          </div>

                          <div class="form-group col-lg-7">
                            <label for="email">{{ ln('email','ucwords') }}</label>
                            <input type="text" name="email"  value="{{ $row->email }}" class="form-control" placeholder="{{ ln('email','ucwords') }}">
                          </div>

                          <div class="form-group col-lg-7">
                            <label for="phone">{{ ln('phone','ucwords') }}</label>
                            <input type="number" name="phone" value="{{ $row->phone }}"  class="form-control" placeholder="{{ ln('phone','ucwords') }}"  maxlength="10" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);">
                          </div>
                          ...
                          <div class="col-lg-7">
                            <div class="form-group">
                              ...
                              <input type="password" name="password"  class="form-control show_pass" placeholder="{{ ln('password','ucwords') }}">
                            </div>
...
<script>
    ...
    $('#FormData').submit(function(e){
      ...
      $.ajax({
          url: "{{route('admin.manage.update',[$row->id,'_token'=>csrf_token()])}}",
          type: "POST",
          data: formdata,
          processData: false,
          contentType: false,

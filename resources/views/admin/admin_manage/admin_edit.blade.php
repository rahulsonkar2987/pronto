                ...
                <div class="col-lg-12">
                    <div class="card text-left">
                      <div class="card-body">
                        ...
                        {!! Form::open(['id'=>'FormData','method'=>'PATCH']) !!}
                          <div class="form-group col-lg-7">
                              <label for="first_name">{{ ln('first_name','ucwords') }}</label>
                              <input type="text" name="first_name" value="{{ $admin->first_name }}"  class="form-control" placeholder="{{ ln('first_name','ucwords') }}">
                          </div>

                          <div class="form-group col-lg-7">
                            <label for="last_name">{{ ln('last_name','ucwords') }}</label>
                            <input type="text" name="last_name"  value="{{ $admin->last_name }}" class="form-control" placeholder="{{ ln('last_name','ucwords') }}">
                          </div>

                          <div class="form-group col-lg-7">
                            <label for="email">{{ ln('email','ucwords') }}</label>
                            <input type="text" name="email"  value="{{ $admin->email }}" class="form-control" placeholder="{{ ln('email','ucwords') }}">
                          </div>

                          <div class="form-group col-lg-7">
                            <label for="phone">{{ ln('phone','ucwords') }}</label>
                            <input type="number" name="phone" value="{{ $admin->phone }}"  class="form-control" placeholder="{{ ln('phone','ucwords') }}"  maxlength="10" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);">
                          </div>
...
<script>
    ...
    $('#FormData').submit(function(e){
      ...
      $.ajax({
          url: "{{route('admin.manage.update',[$admin->id])}}",
          type: "POST",
          data: formdata,
          processData: false,
          contentType: false,
